...
int D_Add(List* all) {
	...
	case 3:
		...
		for (int i = 0; i < number*2; i++) {
			x = rand() % number + 1;//порядковые номера в списке
			y = rand() % number + 1;
			list = all;
			lp = all;
			for (int i = 0; i < x; i++) {
...
int D_Find(List* all) {
	int choice, min, max, mass[30], count = 0, count_turn = 0, total = 0, buf1 = 0;
	char* name, * name_sub, * name_buf = NULL;
	Graph* buf = NULL, * sub, * buf2;
	List* lp, * queue, * lp2, * lp3;
	if (all->sub != NULL) {
		printf("%s", "Ordinary search(1) or breadth-first search(2) or by dijkstra's algorithm(3) or residual network(4)\n");
		getInt(&choice);
		switch (choice)
		{
		case 1:
		...
		case 3:
			...
			count = dijkstra(all, sub, buf2);
			if(count == -1)
				printf("%s", "There is no path between these vertexes\n");
			else
			printf("%s%d%c", "Smallest distance:", count,'\n');
			break;
		case 4:
			printf("%s", "Enter the name of the first vertex:");
			scanf_s("%*c");
			...
			while (1){
				...
				buf = sub;
				lp = buf->list;
				while (buf != buf2){//случайный путь    
					//нахождение максимального пути
					lp2 = lp;
					max = 0;
					count_turn = 0;
					while (lp != NULL){
						if (lp->remain > max && lp->sub->visited == 0 && buf != lp->sub && lp->remain > 0) {
							max = lp->remain;
							name_buf = lp->sub->name;
						}
						lp = lp->next;
					}
					if (max == 0) {
						...
						lp = sub->list;
						count_turn = 0;
						while (mass[count_turn] != -1) {
							for (int i = 0; i < mass[count_turn]; i++)
								lp = lp->next;
							count_turn++;
							buf = lp->sub;
							lp = lp->sub->list;
						...
						if (count <= 0) { //полный выход
							...
							lp = sub->list;*/
							while (lp != NULL) {
								if (lp->remain > 0 && lp->sub != sub && lp->sub->visited == 0) {
									goto again;
								}
								lp = lp->next;
							}

							printf("%s%d%c", "MAX flow: \n", total, '\n');
							network(all);
							return 1;
						}continue;
					}
					//запоминаем куда повернули и устанавливаем минимальный элемент на пути
					while (lp2->remain!=max || name_buf!=lp2->sub->name){
						count_turn++;
						lp2 = lp2->next;
					}
					mass[count] = count_turn;
					if (lp2->remain < min)
						min = lp2->remain;
					//посещаем вершину
					buf->visited = 1;
				...
				//отнимаем минимальное значение на пути
				lp = sub->list;
				count_turn = 0;
				while (mass[count_turn] != -1) {
					for (int i = 0; i < mass[count_turn]; i++)
				...
				total += min;
				//убираем значения посещенных вершин и поворотов
				lp = all;
				while (lp != NULL){
					lp->sub->visited = 0;
					lp = lp->next;
	...
	if (all->sub != NULL) {
		while (lp != NULL) {
			...
			if (lp->sub->list != NULL) {
				...
				sub = lp->sub->list;
				while (sub->next != NULL) {
					printf("%s%s%u%c", sub->sub->name, " ; ", sub->weight, '\n');
					sub = sub->next;
				}
				printf("%s%s%u%c", sub->sub->name, " ; ", sub->weight, '\n');
			}
...
int D_Timing() {
	...
	for (int i = 0; i < 100; i++) {
		...
		for (int i = 0; i < number*2; i++) {
			x = rand() % total + 1;//порядковые номера в списке
			y = rand() % total + 1;
			list = all;
			lp = all;
			for (int i = 0; i < x; i++) {
...
int delTree(List* all) {
	...
	if (all->sub != NULL) {//элементов нет
		if (all->next == NULL) {//1 элемент
			...
			lp = parent->next;
			while (lp!=NULL){
				if (parent->sub->list != NULL) {//удаление рёбер
